...
const getAdvertisement = async (req, res) => {
    try {
        const allAdvertisement = await advertisement.find().populate("users").populate("jobs").populate("map")
...
const getOneAdvertisement = async (req, res) => {
    try {
        const { id } = req.params
        const oneAdvertisement = await advertisement.findById(id).populate("users").populate("jobs").populate("map")
